...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := lddlrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = lddlutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := lddlutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := lddlutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*lddljson.RPCError).Code !=
		lddljson.ErrRPCWalletPassphraseIncorrect {
		// lddljson.ErrWalletPassphraseIncorrect.Code
	...
	// lddljson.ErrRPCWalletUnlockNeeded
	if !strings.HasPrefix(err.Error(),
		strconv.Itoa(int(lddljson.ErrRPCWalletUnlockNeeded))) {
		t.Fatalf("createnewaccount returned error (%v) instead of %v",
			err, lddljson.ErrRPCWalletUnlockNeeded)
	}
	...
	if err != nil && err.(*lddljson.RPCError).Code !=
		lddljson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	preAccountBalances := make(map[string]lddljson.GetAccountBalanceResult)
	...
	sendAmount := lddlutil.Amount(700000000)
	...
	postAccountBalances := make(map[string]lddljson.GetAccountBalanceResult)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := lddlutil.Amount(700000000)
	...
	// Get *lddlutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = lddlutil.Amount(txIn.ValueIn).ToCoin()
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := lddlutil.Amount(1000000)
	...
	fee := lddlutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []lddlutil.Amount{700000000, 1400000000}
	...
	addressAmounts := make(map[lddlutil.Address]lddlutil.Amount)
	totalAmountToSend := lddlutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	if *txList1[0].TxType != lddljson.LTTTRegular {
	...
	voutAmt := lddlutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := lddlutil.Amount(240000000)
	...
	var sendResult, recvResult lddljson.ListTransactionsResult
	...
	rxtxResults := map[string]lddljson.ListTransactionsResult{
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// lddlrpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := lddlutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := lddlutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := lddlutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(lddlutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// lddlrpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := lddlutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := lddlutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := lddlutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := lddlutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(lddlutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := lddlutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := lddlutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *lddlrpcclient.Client, t *testing.T) *lddljson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *lddlutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, lddlutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *lddlutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, lddlutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *lddlutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *lddlutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *lddlutil.Tx) lddlutil.Amount {
	var totalSpent int64
	...
	return lddlutil.Amount(totalSpent - totalSent)
}

// getOutPointString uses OutPoint.String() to combine the tx hash with vout
// index from a ListUnspentResult.
func getOutPointString(utxo *lddljson.ListUnspentResult) (string, error) {
