...
import (
	...
	"github.com/Legenddigital/lddlwallet/rpctest"
)
...
func main() {
	...
	fmt.Println("Command for node's lddlctl:")
	fmt.Printf("\tlddlctl -u %s -P %s -s %s -c %s\n", cn.User, cn.Pass,
		cn.Host, nodeCertFile)

	cw := primaryHarness.RPCWalletConfig()
	walletCertFile := primaryHarness.RPCWalletCertFile()
	fmt.Println("Command for wallet's lddlctl:")
	fmt.Printf("\tlddlctl -u %s -P %s -s %s -c %s --wallet\n", cw.User, cw.Pass,
