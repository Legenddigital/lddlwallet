...
// Copyright (c) 2015-2016 The Legenddigital developers
...
// RPCClient represents a persistent client connection to a Legenddigital RPC server
// for information regarding the current best block chain.
type RPCClient struct {
	*lddlrpcclient.Client
	connConfig  *lddlrpcclient.ConnConfig // Work around unexported field
...
// operate on the same Legenddigital network as described by the passed chain
// parameters, the connection will be disconnected.
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	disableTLS bool) (*RPCClient, error) {

	client := &RPCClient{
		connConfig: &lddlrpcclient.ConnConfig{
			Host:                 connect,
			Endpoint:             "ws",
			User:                 user,
			Pass:                 pass,
	...
	ntfnCallbacks := &lddlrpcclient.NotificationHandlers{
	...
	rpcClient, err := lddlrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
func (c *RPCClient) Start(ctx context.Context, retry bool) (err error) {
	...
	if err != nil {
		return errors.E(op, errors.E(errors.Op("lddld.jsonrpc.getcurrentnet"), err))
	}
	...
	if err == nil {
		versionResult := versions["lddldjsonrpcapi"]
		serverAPI = semver{
...
// lddlrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// Legenddigital RPC server.  This channel must be continually read or the process
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to lddld processing websocket requests synchronously (see
...
// POSTClient creates the equivalent HTTP POST lddlrpcclient.Client.
func (c *RPCClient) POSTClient() (*lddlrpcclient.Client, error) {
	configCopy := *c.connConfig
	configCopy.HTTPPostMode = true
	client, err := lddlrpcclient.New(&configCopy, nil)
	if err != nil {
