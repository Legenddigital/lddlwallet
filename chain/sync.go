...
// notifications from a lddld JSON-RPC server.
...
// the lddld server are performed.  Otherwise, it will listen for notifications
...
func (s *RPCSyncer) Run(ctx context.Context, startupSync bool) error {
	...
	g.Go(func() error {
		...
		if err != nil {
			const op errors.Op = "lddld.jsonrpc.notifyspentandmissedtickets"
			return errors.E(op, err)
		}
		...
		if s.wallet.VotingEnabled() {
			...
			if err != nil {
				const op errors.Op = "lddld.jsonrpc.notifywinningtickets"
				return errors.E(op, err)
			}
...
func (s *RPCSyncer) handleNotifications(ctx context.Context) error {
	...
	for {
		...
		case n, ok := <-c:
			...
			switch n := n.(type) {
			case blockConnected:
				op = "lddld.jsonrpc.blockconnected"
			...
			case relevantTxAccepted:
				op = "lddld.jsonrpc.relevanttxaccepted"
				nonFatal = true
			...
			case missedTickets:
				op = "lddld.jsonrpc.spentandmissedtickets"
...
func (s *RPCSyncer) handleVoteNotifications(ctx context.Context) error {
	...
	for {
		...
		case n, ok := <-c:
			...
			switch n := n.(type) {
			case winningTickets:
				op = "lddld.jsonrpc.winningtickets"
...
func (s *RPCSyncer) startupSync(ctx context.Context) error {
	...
	if err != nil {
		const op errors.Op = "lddld.jsonrpc.notifyblocks"
		return errors.E(op, err)
	}
	...
	if err != nil {
		const op errors.Op = "lddld.jsonrpc.rebroadcastwinners"
		return errors.E(op, err)
	}
	_, err = s.rpcClient.RawRequest("rebroadcastmissed", nil)
	if err != nil {
		const op errors.Op = "lddld.jsonrpc.rebroadcastmissed"
		return errors.E(op, err)
	}
