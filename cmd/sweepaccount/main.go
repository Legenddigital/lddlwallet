...
var (
	walletDataDirectory = lddlutil.AppDataDir("lddlwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool                `long:"testnet" description:"Use the test Legenddigital network"`
	SimNet                bool                `long:"simnet" description:"Use the simulation Legenddigital network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple Legenddigital networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []lddljson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue   lddlutil.Amount
		inputs            = make([]*wire.TxIn, 0, len(outputs))
		redeemScriptSizes = make([]int, 0, len(outputs))
		sourceErr         error
	)
	for _, output := range outputs {
		outputAmount, err := lddlutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(lddlutil.Amount) (*txauthor.InputDetail, error) {
...
type destinationScriptSourceToAccount struct {
	accountName string
	rpcClient   *lddlrpcclient.Client
}
...
func (src *destinationScriptSourceToAddress) Script() ([]byte, uint16, error) {
	destinationAddress, err := lddlutil.DecodeAddress(src.address)
	if err != nil {
		return nil, 0, err
	}
	script, err := txscript.PayToAddrScript(destinationAddress)
...
func sweep() error {
	...
	rpcClient, err := lddlrpcclient.New(&lddlrpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]lddljson.ListUnspentResult)
	...
	var totalSwept lddlutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := lddlutil.Amount(atx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount lddlutil.Amount) bool {
	return amount >= 0 && amount <= lddlutil.MaxAmount
}

func parseOutPoint(input *lddljson.ListUnspentResult) (wire.OutPoint, error) {
