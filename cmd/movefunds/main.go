...
type configJSON struct {
	...
	DcrctlArgs    string `json:"lddlctlargs"`
}
...
func convertJSONUnspentToOutPoints(
	utxos []lddljson.ListUnspentResult) []*extendedOutPoint {
	...
	for _, utxo := range utxos {
		...
		eop.op = op
		amtCast, _ := lddlutil.NewAmount(utxo.Amount)
...
func main() {
	...
	var utxos []lddljson.ListUnspentResult
	...
	sendToAddress, err := lddlutil.DecodeAddress(cfg.SendToAddress)
	if err != nil {
