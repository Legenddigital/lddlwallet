...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb lddlutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(lddlutil.Amount) (*txauthor.InputDetail, error) {
				inputDetail, err := sourceImpl.SelectInputs(lddlutil.MaxAmount)
...
func (s *secretSource) GetKey(addr lddlutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr lddlutil.Address) ([]byte, error) {
...
func (w *Wallet) checkHighFees(totalInput lddlutil.Amount, tx *wire.MsgTx) error {
	if w.AllowHighFees {
		return nil
	}
...
//
// Legenddigital: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(op errors.Op, outputs []*wire.TxOut, account uint32, minconf int32,
	n NetworkBackend, randomizeChangeIdx bool, txFee lddlutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := lddlutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisig(op errors.Op, account uint32, amount lddlutil.Amount, pubkeys []*lddlutil.AddressSecpPubKey,
	nRequired int8, minconf int32) (*CreatedTx, lddlutil.Address, []byte, error) {

	var (
		ctx      *CreatedTx
		addr     lddlutil.Address
		msScript []byte
	)
	err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		var err error
...
func (w *Wallet) txToMultisigInternal(op errors.Op, dbtx walletdb.ReadWriteTx, account uint32, amount lddlutil.Amount,
	pubkeys []*lddlutil.AddressSecpPubKey, nRequired int8, minconf int32) (*CreatedTx, lddlutil.Address, []byte, error) {

	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	txToMultisigError := func(err error) (*CreatedTx, lddlutil.Address, []byte, error) {
	...
	// fee estimator.
	var feeEstForTx lddlutil.Amount
	...
	scAddr, err := lddlutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
	...
	// Request updates from lddld for new transactions sent to this
	// script hash address.
	err = n.LoadTxFilter(context.TODO(), false, []lddlutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
...
func (w *Wallet) compressWallet(op errors.Op, maxNumIns int, account uint32, changeAddr lddlutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(op errors.Op, dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr lddlutil.Address) (*chainhash.Hash, error) {
...
// passed.
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint, input *extendedOutPoint, addrVote lddlutil.Address,
	addrSubsidy lddlutil.Address, ticketCost int64, addrPool lddlutil.Address) (*wire.MsgTx, error) {
	...
	zeroed := [20]byte{}
	addrZeroed, err := lddlutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			lddlutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		lddlutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
		return nil, errors.E(errors.Op("txscript.GenerateSStxAddrPush"), errors.Invalid,
...
func (w *Wallet) purchaseTickets(op errors.Op, req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(errors.Op, persistReturnedChildFunc, uint32) (lddlutil.Address, error) {
			return addr, err
		}
	}
	...
	var neededPerTicket, ticketFee lddlutil.Amount
	...
	// do so now.
	var poolFeeAmt lddlutil.Amount
	if poolAddress != nil {
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * lddlutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy lddlutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       lddlutil.Amount(eopPool.amt),
		...
		eopCredit := udb.Credit{
			OutPoint:     *eop.op,
			BlockMeta:    udb.BlockMeta{},
			Amount:       lddlutil.Amount(eop.amt),
		...
		err = w.checkHighFees(lddlutil.Amount(eop.amt), ticket)
		if err != nil {
			return ticketHashes, errors.E(op, err)
		}
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount lddlutil.Amount, currentHeight int32) ([]udb.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)
	var outTotal lddlutil.Amount
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr lddlutil.Address) (chainec.PrivateKey, bool, error) {
		...
		return key, true, nil // secp256k1 pubkeys are always compressed in Legenddigital
	}
	var getScript txscript.ScriptClosure = func(addr lddlutil.Address) ([]byte, error) {
	...
	signedScript, err := txscript.SignTxOutput(w.chainParams, tx, inputToSign,
		...
		tx.TxIn[inputToSign].SignatureScript, lddlec.STEcdsaSecp256k1)
	if err != nil {
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB lddlutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if lddlutil.Amount(output.Value) > feeEstimate {
			amount := lddlutil.Amount(output.Value) - feeEstimate
