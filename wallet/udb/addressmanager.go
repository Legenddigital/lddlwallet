...
import (
	...
	"github.com/Legenddigital/lddlwallet/wallet/internal/snacl"
...
func normalizeAddress(addr lddlutil.Address) lddlutil.Address {
...
// now created using the coin types assigned to Legenddigital in SLIP0044:
//
...
// to the coin type registered to Legenddigital as per SLIP0044.  On mainnet, this
...
//
// This function MUST be called with the manager lock held for writes.
func (m *Manager) loadAddress(ns walletdb.ReadBucket, address lddlutil.Address) (ManagedAddress, error) {
...
func (m *Manager) Address(ns walletdb.ReadBucket, address lddlutil.Address) (ManagedAddress, error) {
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket, address lddlutil.Address) (uint32, error) {
...
func (m *Manager) ImportPrivateKey(ns walletdb.ReadWriteBucket, wif *lddlutil.WIF) (ManagedPubKeyAddress, error) {
	defer m.mtx.Unlock()
	m.mtx.Lock()
	...
	pubKeyHash := lddlutil.Hash160(serializedPubKey)
...
func (m *Manager) ImportScript(ns walletdb.ReadWriteBucket, script []byte) (ManagedScriptAddress, error) {
	...
	// Prevent duplicates.
	scriptHash := lddlutil.Hash160(script)
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address lddlutil.Address) error {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr lddlutil.Address) error) error {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *Manager) PrivateKey(ns walletdb.ReadBucket, addr lddlutil.Address) (key chainec.PrivateKey, done func(), err error) {
...
func (m *Manager) RedeemScript(ns walletdb.ReadBucket, addr lddlutil.Address) (script []byte, done func(), err error) {
	// Lock the manager mutex for writes.  This protects read access to m.locked
