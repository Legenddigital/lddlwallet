...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := lddlutil.NewAddressPubKeyHash(pkHash,
...
func testImportPrivateKey(tc *testContext) bool {
	...
	if tc.create {
		for i, test := range tests {
			...
			wif, err := lddlutil.DecodeWIF(test.in)
			if err != nil {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			utilAddr, err := lddlutil.NewAddressPubKeyHash(
...
func testImportScript(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// Use the Address API to retrieve each of the expected
			// new addresses and ensure they're accurate.
			utilAddr, err := lddlutil.NewAddressScriptHash(test.in,
				chainParams)
			if err != nil {
...
func testMarkUsed(tc *testContext) bool {
	...
	for i, test := range tests {
		...
		case addrPubKeyHash:
			...
			addr, err = lddlutil.NewAddressPubKeyHash(addrHash, chainParams, chainec.ECTypeSecp256k1)
		case addrScriptHash:
			testtype = "addrScriptHash"
			addr, err = lddlutil.NewAddressScriptHashFromHash(addrHash, chainParams)
		default:
			panic("unreachable")
		}
		if err != nil {
...
func testLookupAccount(tc *testContext) bool {
	...
	for i, addr := range expectedAddrs {
		addr, err := lddlutil.NewAddressPubKeyHash(addr.addressHash,
