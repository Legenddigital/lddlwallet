...
type ManagedAddress interface {
	...
	// Address returns a lddlutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *lddlutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the lddlutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() lddlutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = lddlutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = lddlutil.Hash160(pubKey.SerializeUncompressed())
	}
	address, err := lddlutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
		lddlec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *lddlutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() lddlutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := lddlutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
