...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the lddlutil.Tx of the read MsgTx and set its index.
		tx := lddlutil.NewTx(msgTx)
		tx.SetIndex(lddlutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: lddlutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
