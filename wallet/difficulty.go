...
// This code was copied from lddld/blockchain/difficulty.go and modified for
// lddlwallet's header storage.
...
func (w *Wallet) ancestorHeaderAtHeight(dbtx walletdb.ReadTx, h *wire.BlockHeader, chain []*BlockNode, height int32) (*wire.BlockHeader, error) {
	...
	case height > int32(h.Height), height < 0:
		return nil, nil // lddld's blockNode.Ancestor returns nil for child heights
	}
...
// in DCP0001.
func (w *Wallet) nextRequiredDCP0001PoSDifficulty(dbtx walletdb.ReadTx, curHeader *wire.BlockHeader, chain []*BlockNode) (lddlutil.Amount, error) {
	...
	if nextHeight < stakeDiffStartHeight {
		return lddlutil.Amount(w.chainParams.MinimumStakeDiff), nil
	}
	...
	if nextHeight%intervalSize != 0 {
		return lddlutil.Amount(curDiff), nil
	}
	...
	return lddlutil.Amount(sdiff), nil
}
...
// trusted full node.
func (w *Wallet) NextStakeDifficulty() (lddlutil.Amount, error) {
	const op errors.Op = "wallet.NextStakeDifficulty"
	var sdiff lddlutil.Amount
...
func (w *Wallet) NextStakeDifficultyAfterHeader(h *wire.BlockHeader) (lddlutil.Amount, error) {
...
func (w *Wallet) validateHeaderChainDifficulties(dbtx walletdb.ReadTx, chain []*BlockNode, idx int) ([]*BlockNode, error) {
	...
	for ; idx < len(chain); idx++ {
		...
		if deployments.DCP0001.Active(int32(h.Height), w.chainParams) {
			...
			if lddlutil.Amount(h.SBits) != sdiff {
				err := errors.Errorf("%v has invalid PoS difficulty, got %v, want %v",
					&hash, lddlutil.Amount(h.SBits), sdiff)
				return chain[idx:], errors.E(op, errors.Consensus, err)
			}
		}
