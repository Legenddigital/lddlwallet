...
func (w *Wallet) StakePoolUserInfo(userAddress lddlutil.Address) (*udb.StakePoolUser, error) {
	const op errors.Op = "wallet.StakePoolUserInfo"

	switch userAddress.(type) {
	case *lddlutil.AddressPubKeyHash: // ok
	case *lddlutil.AddressScriptHash: // ok
	default:
