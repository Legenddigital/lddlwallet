...
import (
	...
	"github.com/Legenddigital/lddld/chaincfg/chainec"
	"github.com/Legenddigital/lddld/chaincfg/chainhash"
	"github.com/Legenddigital/lddld/lddlec"
	"github.com/Legenddigital/lddld/lddlec/secp256k1"
...
type Wallet struct {
	...
	votingEnabled      bool
	balanceToMaintain  lddlutil.Amount
	poolAddress        lddlutil.Address
	poolFees           float64
	...
	relayFee               lddlutil.Amount
	relayFeeMu             sync.Mutex
	ticketFeeIncrementLock sync.Mutex
	ticketFeeIncrement     lddlutil.Amount
	DisallowFree           bool
	AllowHighFees          bool
	...
	addressReuse     bool
	ticketAddress    lddlutil.Address
...
type Config struct {
	...
	VotingEnabled bool
	AddressReuse  bool
	VotingAddress lddlutil.Address
	PoolAddress   lddlutil.Address
	PoolFees      float64
	TicketFee     float64
...
func (w *Wallet) BalanceToMaintain() lddlutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance lddlutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() lddlutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee lddlutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() lddlutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee lddlutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) loadActiveAddrs(ctx context.Context, dbtx walletdb.ReadTx, nb NetworkBackend) (uint64, error) {
	...
	loadBranchAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32, errs chan<- error) {
		...
		for child := uint32(0); child <= n; child += step {
			child := child
			g.Go(func() error {
				addrs := make([]lddlutil.Address, 0, step)
...
func (w *Wallet) CommittedTickets(tickets []*chainhash.Hash) ([]*chainhash.Hash, []lddlutil.Address, error) {
	const op errors.Op = "wallet.CommittedTickets"
	hashes := make([]*chainhash.Hash, 0, len(tickets))
	addresses := make([]lddlutil.Address, 0, len(tickets))
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for _, v := range tickets {
			...
			var bestAddr lddlutil.Address
			var bestAmount lddlutil.Amount
			...
			for i := 1; i < len(tx.TxOut); i += 2 {
				...
				if _, ok := addr.(*lddlutil.AddressPubKeyHash); !ok {
...
// message or lddld JSON-RPC request, for the blocks in sidechain and saved in
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*lddlutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
		resp      chan createMultisigTxResponse
	}
	purchaseTicketRequest struct {
		minBalance  lddlutil.Amount
		spendLimit  lddlutil.Amount
		minConf     int32
		ticketAddr  lddlutil.Address
		account     uint32
		numTickets  int
		poolAddress lddlutil.Address
		poolFees    float64
		expiry      int32
		txFee       lddlutil.Amount
		ticketFee   lddlutil.Amount
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address lddlutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount lddlutil.Amount, pubkeys []*lddlutil.AddressSecpPubKey, nrequired int8, minconf int32) (*CreatedTx, lddlutil.Address, []byte, error) {
...
// tickets.
func (w *Wallet) PurchaseTickets(minBalance, spendLimit lddlutil.Amount, minConf int32, ticketAddr lddlutil.Address, account uint32, numTickets int, poolAddress lddlutil.Address,
	poolFees float64, expiry int32, txFee lddlutil.Amount, ticketFee lddlutil.Amount) ([]*chainhash.Hash, error) {
...
// spending to it in the blockchain or lddld mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (lddlutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a lddlutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr lddlutil.Address) (sig []byte, err error) {
	const op errors.Op = "wallet.SignMessage"
	var buf bytes.Buffer
	wire.WriteVarString(&buf, 0, "Legenddigital Signed Message:\n")
...
func VerifyMessage(msg string, addr lddlutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := lddlutil.NewAddressSecpPubKey(serializedPK, addr.Net())
	if err != nil {
		return false, errors.E(op, err)
	}
...
func (w *Wallet) HaveAddress(a lddlutil.Address) (bool, error) {
...
func (w *Wallet) AccountOfAddress(a lddlutil.Address) (uint32, error) {
...
func (w *Wallet) AddressInfo(a lddlutil.Address) (udb.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager, syncHeight int32, net *chaincfg.Params) (sends, receives []lddljson.ListTransactionsResult) {
	addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
	...
	txTypeStr := lddljson.LTTTRegular
	switch details.TxType {
	case stake.TxTypeSStx:
		txTypeStr = lddljson.LTTTTicket
	case stake.TxTypeSSGen:
		txTypeStr = lddljson.LTTTVote
	case stake.TxTypeSSRtx:
		txTypeStr = lddljson.LTTTRevocation
	}

	// Fee can only be determined if every input is a debit.
	var feeF64 float64
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal lddlutil.Amount
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := lddlutil.Amount(output.Value).ToCoin()
		result := lddljson.ListTransactionsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   BlockIndex
			//
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]lddljson.ListTransactionsResult, error) {
	const op errors.Op = "wallet.ListSinceBlock"
	txList := []lddljson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]lddljson.ListTransactionsResult, error) {
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]lddljson.ListTransactionsResult, error) {
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*lddlutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]lddljson.ListTransactionsResult, error) {
...
// transaction.
func (w *Wallet) ListTransactionDetails(txHash *chainhash.Hash) ([]lddljson.ListTransactionsResult, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		txList = make([]lddljson.ListTransactionsResult, 0, len(sends)+len(receives))
		txList = append(txList, receives...)
		txList = append(txList, sends...)
		return nil
	})
...
func (w *Wallet) GetTicketsPrecise(f func([]*TicketSummary, *wire.BlockHeader) (bool, error), chainClient *lddlrpcclient.Client, startBlock, endBlock *BlockIdentifier) error {
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*lddlutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*lddljson.ListUnspentResult, error) {
	const op errors.Op = "wallet.ListUnspent"
	var results []*lddljson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		scSwitch:
			...
			result := &lddljson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr lddlutil.Address) (string, error) {
	...
	wif, err := lddlutil.NewWIF(privKey, w.chainParams, lddlec.SignatureType(privKey.GetType()))
	if err != nil {
		return "", errors.E(op, err)
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *lddlutil.WIF) (string, error) {
	...
	if n, err := w.NetworkBackend(); err == nil {
		err := n.LoadTxFilter(context.TODO(), false, []lddlutil.Address{addr}, nil)
		if err != nil {
			return "", errors.E(op, err)
		}
	}
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr lddlutil.Address) ([]byte, error) {
...
type StakeInfoData struct {
	BlockHeight  int64
	TotalSubsidy lddlutil.Amount
	Sdiff        lddlutil.Amount
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += lddlutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) StakeInfoPrecise(chainClient *lddlrpcclient.Client) (*StakeInfoData, error) {
	const op errors.Op = "wallet.StakeInfoPrecise"
	// This is only needed for the total count and can be optimized.
	mempoolTicketsFuture := chainClient.GetRawMempoolAsync(lddljson.GRMTickets)
...
func (w *Wallet) LockedOutpoints() []lddljson.TransactionInput {
	locked := make([]lddljson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = lddljson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr lddlutil.Address) error {
...
// returning the total amount of Legenddigital received for all accounts.
...
// returning the total amount of Legenddigital received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr lddlutil.Address, minConf int32) (lddlutil.Amount, error) {
	const op errors.Op = "wallet.TotalReceivedForAddr"
	var amount lddlutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType, additionalPrevScripts map[wire.OutPoint][]byte,
	additionalKeysByAddress map[string]*lddlutil.WIF, p2shRedeemScriptsByAddress map[string][]byte) ([]SignatureError, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr lddlutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr lddlutil.Address) ([]byte, error) {
			...
			if (hashType&txscript.SigHashSingle) !=
				...
				ecType := lddlec.STEcdsaSecp256k1
...
// address.
func (w *Wallet) CreateSignature(tx *wire.MsgTx, idx uint32, addr lddlutil.Address, hashType txscript.SigHashType, prevPkScript []byte) (sig, pubkey []byte, err error) {
...
func Open(cfg *Config) (*Wallet, error) {
	...
	// Amounts
	w.ticketFeeIncrement, err = lddlutil.NewAmount(cfg.TicketFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
	w.relayFee, err = lddlutil.NewAmount(cfg.RelayFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
