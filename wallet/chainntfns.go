...
func (w *Wallet) evaluateStakePoolTicket(rec *udb.TxRecord, blockHeight int32, poolUser lddlutil.Address) bool {
...
func (w *Wallet) processTransactionRecord(dbtx walletdb.ReadWriteTx, rec *udb.TxRecord, header *wire.BlockHeader, blockMeta *udb.BlockMeta) error {
	...
	if stake.IsSStx(&rec.MsgTx) {
		...
		if insert {
			err := w.StakeMgr.InsertSStx(stakemgrNs, lddlutil.NewTx(&rec.MsgTx))
			if err != nil {
				...
				case err == nil:
					if n, err := w.NetworkBackend(); err == nil {
						...
						err := n.LoadTxFilter(context.TODO(),
							false, []lddlutil.Address{addr}, nil)
						if err != nil {
							return errors.E(op, err)
						}
					}
...
func (w *Wallet) VoteOnOwnedTickets(winningTicketHashes []*chainhash.Hash, blockHash *chainhash.Hash, blockHeight int32) error {
	...
	if err != nil {
		...
		rpcErr, ok := err.(*lddljson.RPCError)
		if !ok || rpcErr.Code != lddljson.ErrRPCDuplicateTx {
...
func (w *Wallet) RevokeOwnedTickets(missedTicketHashes []*chainhash.Hash) error {
	...
	err = walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, ticketHash := range ticketHashes {
			...
			err = w.checkHighFees(lddlutil.Amount(ticketPurchase.TxOut[0].Value), revocation)
			if err != nil {
