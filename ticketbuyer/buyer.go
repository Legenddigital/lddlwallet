...
type TicketPurchaser struct {
	...
	lddldChainSvr     *lddlrpcclient.Client
...
func (t *TicketPurchaser) BalanceToMaintain() lddlutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = lddlutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() lddlutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = lddlutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() lddlutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = lddlutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() lddlutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = lddlutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// VotingAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) VotingAddress() lddlutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetVotingAddress(votingAddress lddlutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() lddlutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress lddlutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: lddlutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            lddlutil.Amount(cfg.MaxFee),
		minFee:            lddlutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  lddlutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  lddlutil.Amount
	PriceNext     lddlutil.Amount
	PriceCurrent  lddlutil.Amount
	Purchased     int
	LeftWindow    int
	Balance       lddlutil.Amount
	TicketPrice   lddlutil.Amount
}
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		...
		log.Debugf("Transaction relay fee: %v", lddlutil.Amount(t.cfg.TxFee))
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfoPrecise(t.lddldChainSvr)
			if err != nil {
		...
		t.stakeVoteSubsidy = lddlutil.Amount(subsidy)
	...
	if avgPriceAmt < lddlutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = lddlutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	if err != nil && t.lddldChainSvr != nil {
		// Wallet failed to calculate sdiff (DCP0001 may not be active), so
		// query it over RPC.
		sd, err := t.lddldChainSvr.GetStakeDifficulty()
		if err == nil {
			nextStakeDiff, err = lddlutil.NewAmount(sd.NextStakeDifficulty)
			if err != nil {
				return ps, err
			}
		}
	...
	sDiffEsts, err := t.lddldChainSvr.EstimateStakeDiff(nil)
	if err == nil {
		ps.PriceNext, err = lddlutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt lddlutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.lddldChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	// have enough moneys.
	notEnough := func(bal lddlutil.Amount, toBuy int, sd lddlutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := lddlutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
