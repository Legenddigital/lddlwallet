...
type diffPeriodFee struct {
	difficulty lddlutil.Amount
	difference lddlutil.Amount // Difference from current difficulty
	fee        lddlutil.Amount
}
...
func (t *TicketPurchaser) findClosestFeeWindows(difficulty lddlutil.Amount,
	useMedian bool) (lddlutil.Amount, error) {
	wtcUint32 := uint32(windowsToConsider)
	info, err := t.lddldChainSvr.TicketFeeInfo(&zeroUint32, &wtcUint32)
	if err != nil {
		return 0.0, err
	}
	...
	for i := range info.FeeInfoWindows {
		...
		blH, err := t.lddldChainSvr.GetBlockHash(startHeight)
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.lddldChainSvr.GetBlockHeader(blH)
		if err != nil {
			return 0, err
		}

		windowDiffAmt := lddlutil.Amount(blkHeader.SBits)

		var fee lddlutil.Amount
		if !useMedian {
			fee, err = lddlutil.NewAmount(info.FeeInfoWindows[i].Mean)
		} else {
			fee, err = lddlutil.NewAmount(info.FeeInfoWindows[i].Median)
		}
		if err != nil {
			return 0, err
		}
...
func (t *TicketPurchaser) findTicketFeeBlocks(useMedian bool) (lddlutil.Amount, error) {
	btaUint32 := uint32(t.cfg.BlocksToAvg)
	info, err := t.lddldChainSvr.TicketFeeInfo(&btaUint32, nil)
	if err != nil {
		return 0.0, err
	}

	var sum, tmp lddlutil.Amount
	for i := range info.FeeInfoBlocks {
		if !useMedian {
			tmp, err = lddlutil.NewAmount(info.FeeInfoBlocks[i].Mean)
		} else {
			tmp, err = lddlutil.NewAmount(info.FeeInfoBlocks[i].Median)
		}
		if err != nil {
			return 0, err
		}
	...
	return sum / lddlutil.Amount(t.cfg.BlocksToAvg), nil
}
