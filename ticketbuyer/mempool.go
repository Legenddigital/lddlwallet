...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.votingAddress != nil {
		tiHashes, err := t.lddldChainSvr.GetRawMempool(lddljson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.lddldChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
			...
			addr, err := lddlutil.DecodeAddress(addrStr)
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.lddldChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
