...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (lddlutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt lddlutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		poolValue, err := t.lddldChainSvr.GetTicketPoolValue()
		if err != nil {
			return 0, err
		}
		bestBlockH, err := t.lddldChainSvr.GetBestBlockHash()
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.lddldChainSvr.GetBlockHeader(bestBlockH)
		if err != nil {
			return 0, err
		}
		...
		avgPricePoolAmt = poolValue / lddlutil.Amount(poolSize)
	...
	var ticketVWAP lddlutil.Amount
	if t.priceMode == AvgPriceVWAPMode || t.priceMode == AvgPriceDualMode {
		...
		for i := 0; i < vwapReqTries; i++ {
			ticketVWAP, err = t.lddldChainSvr.TicketVWAP(&startVWAPHeight,
				&endVWAPHeight)
