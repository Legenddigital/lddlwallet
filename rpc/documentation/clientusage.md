...
var certificateFile = filepath.Join(lddlutil.AppDataDir("lddlwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", lddlutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/lddlwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/lddlwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by lddlwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.lddlwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\lddlwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by lddlwallet).
...
if (os.platform() == 'win32') {
  ...
  certPath = path.join(process.env.HOME, '.lddlwallet', 'rpc.cert');
}
...
```bash
$ protoc -I /path/to/Legenddigital/lddlwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    if platform.system() == 'Windows':
        ...
        cert_file_path = os.path.join(os.environ['HOME'], '.lddlwallet', 'rpc.cert')
