...
the management of the wallet and its connection to the Legenddigital network.  It has
...
dynamically start the lddld RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the Legenddigital network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default lddld RPC port of the
  active Legenddigital network.
...
- `string public_key`: The public key encoded as a string in the Legenddigital encoding
  format.
...
the Legenddigital network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of lddld. Right now, wallet block processing
