...
// Copyright (c) 2016-2017 The Legenddigital developers
...
//
//   https://github.com/Legenddigital/lddlwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/Legenddigital/lddlwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/Legenddigital/lddld/addrmgr"
	...
	"github.com/Legenddigital/lddlwallet/internal/zero"
	"github.com/Legenddigital/lddlwallet/loader"
	"github.com/Legenddigital/lddlwallet/netparams"
	"github.com/Legenddigital/lddlwallet/p2p"
	pb "github.com/Legenddigital/lddlwallet/rpc/walletrpc"
	"github.com/Legenddigital/lddlwallet/spv"
	...
	"github.com/Legenddigital/lddlwallet/walletseed"
)
...
// lddlutil.DecodeAddress, which does not perform the network check.
func decodeAddress(a string, params *chaincfg.Params) (lddlutil.Address, error) {
	addr, err := lddlutil.DecodeAddress(a)
	if err != nil {
...
// as well as establishing a RPC connection to a lddld consensus server.
...
func (s *walletServer) NextAddress(ctx context.Context, req *pb.NextAddressRequest) (
	*pb.NextAddressResponse, error) {
	...
	pubKeyAddr, err := lddlutil.NewAddressSecpPubKey(pubKey.Serialize(), s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := lddlutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
...
func (s *walletServer) ImportScript(ctx context.Context,
	...
	p2sh, err := lddlutil.NewAddressScriptHash(req.Script, s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) BlockInfo(ctx context.Context, req *pb.BlockInfoRequest) (*pb.BlockInfoResponse, error) {
	...
	return &pb.BlockInfoResponse{
		...
		ApprovesParent:   header.VoteBits&lddlutil.BlockValid != 0,
	}, nil
}
...
func (s *walletServer) UnspentOutputs(req *pb.UnspentOutputsRequest, svr pb.WalletService_UnspentOutputsServer) error {
	...
	inputDetail, err := s.wallet.SelectInputs(lddlutil.Amount(req.TargetAmount), policy)
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	if req.IncludeChangeScript && inputDetail.Amount > lddlutil.Amount(req.TargetAmount) {
...
func (s *walletServer) ConstructTransaction(ctx context.Context, req *pb.ConstructTransactionRequest) (
	*pb.ConstructTransactionResponse, error) {
	...
	if req.FeePerKb != 0 {
		feePerKb = lddlutil.Amount(req.FeePerKb)
	}
...
func (s *walletServer) PurchaseTickets(ctx context.Context,
	...
	// purchase request.
	spendLimit := lddlutil.Amount(req.SpendLimit)
	if spendLimit < 0 {
		return nil, status.Errorf(codes.InvalidArgument,
	...
	txFee := lddlutil.Amount(req.TxFee)
	ticketFee := s.wallet.TicketFeeIncrement()

	// Set the ticket fee if specified
	if req.TicketFee > 0 {
		ticketFee = lddlutil.Amount(req.TicketFee)
	}
...
func (s *walletServer) ValidateAddress(ctx context.Context, req *pb.ValidateAddressRequest) (*pb.ValidateAddressResponse, error) {
	...
	case udb.ManagedPubKeyAddress:
		...
		pubKeyAddr, err := lddlutil.NewAddressSecpPubKey(result.PubKey,
...
func marshalBlock(v *wallet.Block) *pb.BlockDetails {
	...
	return &pb.BlockDetails{
		...
		ApprovesParent: v.Header.VoteBits&lddlutil.BlockValid != 0,
		Transactions:   txs,
	}
}
...
func (s *messageVerificationServer) VerifyMessage(ctx context.Context, req *pb.VerifyMessageRequest) (
	*pb.VerifyMessageResponse, error) {
	...
	addr, err := lddlutil.DecodeAddress(req.Address)
	if err != nil {
		return nil, translateError(err)
	}
