...
// method.  This may be a request that is handled directly by lddlwallet, or
// a chain server request that is handled by passing the request down to lddld.
//
...
func (s *Server) handlerClosure(ctx context.Context, request *lddljson.Request) lazyHandler {
...
// but this is only implemented here to work around an oddity with lddljson,
...
func (s *Server) invalidAuth(req *lddljson.Request) bool {
	cmd, err := lddljson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*lddljson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := lddljson.MarshalResponse(req.Jsonrpc, req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := lddljson.MarshalResponse(req.Jsonrpc, req.ID, nil, lddljson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	case "stop":
		...
		stop = true
		res = "lddlwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(ctx, &req)()
	}

	// Marshal and send.
	mresp, err := lddljson.MarshalResponse(req.Jsonrpc, req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
