...
import (
	...
	"github.com/Legenddigital/lddlwallet/internal/cfgutil"
	"github.com/Legenddigital/lddlwallet/netparams"
	"github.com/Legenddigital/lddlwallet/ticketbuyer"
	"github.com/Legenddigital/lddlwallet/version"
...
const (
	defaultCAFilename          = "lddld.cert"
	defaultConfigFilename      = "lddlwallet.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "lddlwallet.log"
	...
	defaultMaxFee                    lddlutil.Amount = 1e6
	defaultMinFee                    lddlutil.Amount = 1e5
...
var (
	lddldDefaultCAFile  = filepath.Join(lddlutil.AppDataDir("lddld", false), "rpc.cert")
	defaultAppDataDir  = lddlutil.AppDataDir("lddlwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of lddld RPC server to connect to"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with lddld"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	LddldUsername     string                  `long:"LddldUsername" description:"Username for lddld authentication"`
	LddldPassword     string                  `long:"LddldPassword" default-mask:"-" description:"Password for lddld authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and lddld authentication (if LddldUsername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and lddld authentication (if LddldPassword is unset)"`
...
//
// The above results in lddlwallet functioning properly without any config
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local lddld cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local lddld and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					lddldCertExists, err := cfgutil.FileExists(
						lddldDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if lddldCertExists {
						cfg.CAFile.Value = lddldDefaultCAFile
					}
				}
			}
		}
	...
	// If the lddld username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for lddld and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for lddld and wallet.
	if cfg.LddldUsername == "" {
		cfg.LddldUsername = cfg.Username
	}
	if cfg.LddldPassword == "" {
		cfg.LddldPassword = cfg.Password
	}
